...
class BSONCollectionSpec extends Specification {
  ...
  "BSONCollection" should {
    "write five docs with success" in { implicit ee: EE =>
    ...
    "read empty cursor" >> {
      @inline def cursor(implicit ee: EE): Cursor[BSONDocument] =
    ...
    "read a doc with success" in { implicit ee: EE =>
    ...
    "read all with success" >> {
      implicit val reader = PersonReader
      @inline def cursor(implicit ee: EE) =
    ...
    "read a doc with error" in { implicit ee: EE =>
    ...
    "read docs until error" in { implicit ee: EE =>
      implicit val reader = new SometimesBuggyPersonReader
    ...
    "read docs skipping errors" in { implicit ee: EE =>
      implicit val reader = new SometimesBuggyPersonReader
      val enumerator = collection.find(BSONDocument()).
    ...
    "read docs skipping errors using collect" in { implicit ee: EE =>
      implicit val reader = new SometimesBuggyPersonReader
